...
export const buyPaymentMethodAction = async (ctx: BotContext) => {
    try {
        ...
        const contractsButtons: InlineKeyboardButton[][] = contracts.map(
            contract => {
                return [
                    {
                        callback_data: QueryTriggers.BUY_CONTRACT(contract.id),
        ...
        console.error('[BUY_PAYMENT_METHOD] Error:', error)
        return ctx.answerCbQuery('Произошла ошибка при обработке запроса', { show_alert: true })
    }
} 